...
def extract():
    ...
    for row in rows:
        ...
        if fields:
            ...
            location = fields[5].string
            cik = fields[7].string
            founded = fields[8].string
            records.append([symbol, name, sector,location,cik,founded])

    header = ['Symbol', 'Name', 'Sector', 'Location','CIK','Founded']
    writer = csv.writer(open('500.csv', 'w'), lineterminator='\n')
